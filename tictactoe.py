...
class PerformanceSystem:
    def __init__(self,game,hypothesis,mode = 'X'):
        self.game = game
    ...
    def performEvaluation(self, board = None):
        ...
        w0,w1,w2,w3,w4,w5,x6 = self.hypothesis
...
class Critic:
    ...
    def getTrainingExamples(self):
        ...
        for i in range(len(history)):
            ...
            board_features = self.game.getFeatures(history[i])
            if winner == mode:
                ...
                if empty_squares == 0:
                    ...
                    if i+2 >= len(history):
                        if self.game.getWinner(history[len(history)-1]) == 0:
                            training_examples.append([board_features,0])
                        else:
                            training_examples.append([board_features,-100])
                    else:
...
class Generalizer:
    ...
    def updateHypothesis(self,history,training_examples):
        for i in range(len(history)):
            ...
            v_eval = self.performEvaluation(history[i])
...
def train():
    ...
    generalizer = Generalizer(hypothesis)
    ...
    for i in range(1000):
        ...
        print("Percentage Won: " + str((learner_wins / total_games) * 100) + "\n")
